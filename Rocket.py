...
class RocketLauncher(heavy.Heavy):
        def __init__(self,rocketLauncherList = "",rocketLauncherAttributes = "", rocket = ""):
                ...
                if rocketLaucherList == "":
                        for i in range (0,3):
                                rocketLaucherList = random.choice(['Vacuum','Tripod','Third Eye','Surplus','Snapshot','Smart Drift Control','Single Point Sling','Quickdraw','Perfect Balence','Linear Compensator','Hard Launch','Flared Magwell','Fitted Stock','Field Scout','Army of One','Aggressive Launch'])
                if rocket == "":
